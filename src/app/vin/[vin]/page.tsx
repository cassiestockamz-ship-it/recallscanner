...
/**
 * Outer page component — renders the chrome (breadcrumb, JSON-LD,
 * cross-links, "check another") instantly and defers the NHTSA round-trip
 * to <VinResultBody /> inside a <Suspense> so users see a skeleton
...
export default async function VinPage({ params }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10 md:py-14">
      ...
      <Suspense key={vinUpper} fallback={<SafetyVerdictSkeleton />}>
        <VinResultBody vin={vinUpper} />
      </Suspense>

      {/* Cross-links (render instantly, outside the Suspense boundary) */}
...
/**
 * The NHTSA-dependent render. Lives inside a <Suspense> boundary so the
 * outer page streams to the browser before this block finishes.
 */
async function VinResultBody({ vin }: { vin: string }) {
  const validFormat = isValidVinFormat(vin);

  const [decoded, recallResult] = validFormat
    ? await Promise.all([decodeVin(vin), getRecallsByVin(vin)])
  ...
  if (!validFormat) return <InvalidState vin={vin} />;
  if (validFormat && !vinRecognized && !apiError) return <UnknownVinState vin={vin} />;
  if (validFormat && apiError) return <ApiErrorState vin={vin} />;
