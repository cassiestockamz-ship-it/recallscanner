...
export default async function VinPage({ params }: Props) {
  ...
  const [decoded, recalls] = await Promise.all([
    decodeVin(vinUpper),
    getRecallsByVin(vinUpper),
  ]);

  const hasRecalls = recalls.length > 0;
  const vehicleName = decoded
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <nav className="text-sm text-slate-400 mb-6">
        ...
        <span className="text-slate-700">VIN Check</span>
      </nav>
            ...
            <>
              ...
              <div>
                ...
                <div className="text-sm text-red-700">
                  {vehicleName ? `Your ${vehicleName} has` : "This vehicle has"} unresolved safety recalls. Contact your dealer for free repairs.
                </div>
              </div>
            </>
          ) : (
            <>
              ...
              <div>
                ...
                <div className="text-sm text-green-700">
                  {vehicleName ? `Your ${vehicleName} has` : "This vehicle has"} no unresolved safety recalls in the NHTSA database.
                </div>
              </div>
            </>
          )}
