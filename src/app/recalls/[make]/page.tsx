...
export default async function MakePage({ params }: Props) {
  ...
  // Group by model
  const modelMap = new Map<string, Recall[]>();
  for (const r of recalls) {
    const m = r.Model || "Unknown";
    if (!modelMap.has(m)) modelMap.set(m, []);
    modelMap.get(m)!.push(r);
  }
  const models = Array.from(modelMap.entries()).sort((a, b) => b[1].length - a[1].length);

  // Recent recalls (last 20)
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <p className="text-slate-500 mb-8">
        {recalls.length.toLocaleString()} total recall campaigns found for {make} vehicles.
        ...
        {models.map(([model, recs]) => (
          ...
          >
            ...
            <div className="text-xs text-slate-400 mt-1">
              {recs.length} recall{recs.length !== 1 ? "s" : ""}
            </div>
          </Link>
        ))}
      </div>
